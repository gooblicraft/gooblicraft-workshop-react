...
const HorizontableBlock = ({ availableTextures = [], onAddBlock, onFormChange }) => {
  ...
  const generateBlockJson = () => {
    ...
    const name = blockData.blockName || 'connected_block';
    const displayName = blockData.blockDisplayName || 'Connected Block';
    const geo = blockData.blockGeometry || 'connected_block';
    ...
    return {
      ...
      "minecraft:block": {
        "description": {
          ...
          "states": {
            "gd:north_neighbor": [0, 1],
            "gd:east_neighbor": [0, 1],
            "gd:south_neighbor": [0, 1],
            "gd:west_neighbor": [0, 1]
          }
        },
        "permutations": [
          {
          ...
          {
            "condition": "(query.block_state('minecraft:cardinal_direction') == 'north' && query.block_state('gd:east_neighbor') && !query.block_state('gd:west_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'south' && query.block_state('gd:west_neighbor') && !query.block_state('gd:east_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'west' && query.block_state('gd:north_neighbor') && !query.block_state('gd:south_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'east' && query.block_state('gd:south_neighbor') && !query.block_state('gd:north_neighbor'))",
            "components": {
              "minecraft:geometry": {
          ...
          {
            "condition": "(query.block_state('minecraft:cardinal_direction') == 'north' || query.block_state('minecraft:cardinal_direction') == 'south') && (query.block_state('gd:east_neighbor') && query.block_state('gd:west_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'west' || query.block_state('minecraft:cardinal_direction') == 'east') && (query.block_state('gd:north_neighbor') && query.block_state('gd:south_neighbor'))",
            "components": {
              "minecraft:geometry": {
                "identifier": `geometry.${geo}`,
                "bone_visibility": {
          ...
          {
            "condition": "(query.block_state('minecraft:cardinal_direction') == 'north' && query.block_state('gd:west_neighbor') && !query.block_state('gd:east_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'south' && query.block_state('gd:east_neighbor') && !query.block_state('gd:west_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'west' && query.block_state('gd:south_neighbor') && !query.block_state('gd:north_neighbor')) || (query.block_state('minecraft:cardinal_direction') == 'east' && query.block_state('gd:north_neighbor') && !query.block_state('gd:south_neighbor'))",
            "components": {
              "minecraft:geometry": {
                "identifier": `geometry.${geo}`,
                "bone_visibility": {
        ...
        "components": {
          ...
          "minecraft:collision_box": parseBoxData(blockData.collisionBox, defaultSelectionBox),
          "minecraft:selection_box": parseBoxData(blockData.selectionBox, defaultSelectionBox),
          "minecraft:light_dampening": 0,
          "minecraft:light_emission": 0,
          "minecraft:custom_components": [
            "gd:horizontal_full_connected",
            "gd:horizontal_full_destroy"
          ]
        }
      }
    };
  ...
  const copyToClipboard = () => {
    ...
    alert("Connected Block JSON copied to clipboard!");
  };
      ...
      <div className="workspace-shell">
        ...
        <div className="workspace-hero" style={{ textAlign: 'center', paddingBottom: '5px', paddingTop: '0px' }}>
          <div>
            ...
            <h2 className="workspace-title">Connected Block Generator</h2>
          </div>
        </div>
            ...
            <section className="workspace-panel" style={{ padding: '30px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                ...
                <label className="workspace-label" style={labelStyle}>
                  ...
                  <input 
                    ...
                    placeholder="connected_block" 
                  />
                </label>

                <label className="workspace-label" style={labelStyle}>
                  <span style={{ marginBottom: '8px' }}>Display Name:</span>
                  <input type='text' className="workspace-input" style={{ width: '100%' }} value={blockData.blockDisplayName} onChange={(e) => handleUpdate('blockDisplayName', e.target.value)} placeholder="Connected Block" />
                </label>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '24px' }}>
                ...
                <label className="workspace-label" style={{ ...labelStyle, gridColumn: 'span 2' }}>
                  <span style={{ marginBottom: '8px' }}>Block Model (Geometry):</span>
                  <div 
              ...
              <div style={{ border: '1px dashed #444c56', borderRadius: '16px', padding: '30px', textAlign: 'center', marginTop: '10px' }}>
                <h3 style={{ margin: '0 0 10px 0', fontSize: '1.2rem', textTransform: 'uppercase' }}>Custom Selection & Collision Box Overrides</h3>
                <p style={{ opacity: 0.7, fontSize: '0.85rem', marginBottom: '24px' }}>Customize box coordinates or leave empty for default full block.</p>
...
export default HorizontableBlock;