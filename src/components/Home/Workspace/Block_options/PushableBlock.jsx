...
const PushableBlock = ({ availableTextures = [], onAddBlock, onFormChange }) => {
  ...
  const generateBlockJson = () => {
    ...
    const name = blockData.blockName || 'retail_grocery_block';
    const displayName = blockData.blockDisplayName || 'Custom Retail Grocery Block';
    ...
    const variant0SelectionBox = '{\n  "origin": [-3, 0, -2],\n  "size": [5, 6.25, 4]\n}';
    const variant123SelectionBox = '{\n  "origin": [-6, 0, 7],\n  "size": [12, 14, 1]\n}';
        ...
        "permutations": [
          {
            ...
            "components": {
              ...
              "minecraft:selection_box": parseBoxData(blockData.selectionBox, variant0SelectionBox)
            }
          },
          {
            ...
            "components": {
              ...
              "minecraft:selection_box": parseBoxData(blockData.selectionBox, variant123SelectionBox)
            }
          },
          {
          ...
          {
            "condition": "q.block_state('goobli:variant') == 3",
            "components": {
              "minecraft:geometry": {
                "identifier": `geometry.${geo}`,
                "bone_visibility": {
  ...
  const getFormattedJsonString = () => {
    ...
    const permutations = jsonObj["minecraft:block"].permutations;
    const permSelectionBoxes = [];

    permutations.forEach((perm, i) => {
      if (perm.components && perm.components["minecraft:selection_box"]) {
        permSelectionBoxes.push({ index: i, box: perm.components["minecraft:selection_box"] });
        perm.components["minecraft:selection_box"] = `@@PERM_BOX_${i}@@`;
      }
    });
    ...
    permSelectionBoxes.forEach((item) => {
      const lineBox = typeof item.box === 'object' ? JSON.stringify(item.box) : item.box;
      rawStr = rawStr.replace(`"@@PERM_BOX_${item.index}@@"`, lineBox);
    });
  ...
  const copyToClipboard = () => {
    ...
    alert("Retail Variant Block JSON copied to clipboard!");
  };
      ...
      <div className="workspace-shell">
        ...
        <div className="workspace-hero" style={{ textAlign: 'center', paddingBottom: '5px', paddingTop: '0px' }}>
          <div>
            ...
            <h2 className="workspace-title">Retail Variant Block Generator</h2>
          </div>
        </div>
            ...
            <section className="workspace-panel" style={{ padding: '30px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
                ...
                <label className="workspace-label" style={labelStyle}>
                  ...
                  <input type='text' className="workspace-input" style={{ width: '100%' }} value={blockData.blockDisplayName} onChange={(e) => handleUpdate('blockDisplayName', e.target.value)} placeholder="Custom Retail Block" />
                </label>
              </div>
              ...
              <div style={{ border: '1px dashed #444c56', borderRadius: '16px', padding: '30px', textAlign: 'center', marginTop: '10px' }}>
                <h3 style={{ margin: '0 0 10px 0', fontSize: '1.2rem', textTransform: 'uppercase' }}>Custom Selection Box Override (Variant 0)</h3>
                <p style={{ opacity: 0.7, fontSize: '0.85rem', marginBottom: '24px' }}>Customize selection box coordinates specifically for variant 0 or leave empty for default.</p>
                
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' }}>
                  <label className="workspace-label" style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
                    <span style={{ marginBottom: '8px', fontWeight: 'bold' }}>Selection Box (Variant 0):</span>
                    <textarea 
                      className="workspace-input"
                      ...
                      placeholder='{"origin": [-3, 0, -2], "size": [5, 6.25, 4]}'
                    />
                  </label>
                </div>
              </div>
